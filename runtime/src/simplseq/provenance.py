...
def write_versions(path: Path, *, root: Path, env: dict[str, str]) -> None:
    commands = {
        ...
        "dada2": ["Rscript", "-e", 'library(dada2); cat(as.character(packageVersion("dada2")), "\\n")'],
    ...
    with ThreadPoolExecutor(max_workers=len(commands)) as executor:
        futures = {
            name: executor.submit(run_version, command, cwd=root, env=env)
...
def write_input_md5s(path: Path, samples: Path) -> None:
    cached: dict[tuple[str, int, int], str] = {}
    if path.exists():
        try:
            with path.open(newline="", encoding="utf-8") as handle:
                for row in csv.DictReader(handle):
                    key = (row.get("file", ""), int(row.get("size_bytes", "")), int(row.get("mtime_ns", "")))
                    if row.get("md5"):
                        cached[key] = row["md5"]
        except (OSError, TypeError, ValueError):
            cached = {}

    fastqs = fastq_paths_from_samples(samples)

    def checksum_row(fastq: Path) -> dict[str, str | int]:
        if not fastq.exists():
            return {"file": str(fastq), "size_bytes": "", "mtime_ns": "", "md5": "missing"}
        stat = fastq.stat()
        key = (str(fastq), stat.st_size, stat.st_mtime_ns)
        return {
            "file": str(fastq),
            "size_bytes": stat.st_size,
            "mtime_ns": stat.st_mtime_ns,
            "md5": cached.get(key) or md5_file(fastq),
        }

    with ThreadPoolExecutor(max_workers=min(4, max(1, len(fastqs)))) as executor:
        rows = list(executor.map(checksum_row, fastqs))

    path.parent.mkdir(parents=True, exist_ok=True)
    with path.open("w", newline="", encoding="utf-8") as handle:
        writer = csv.DictWriter(handle, fieldnames=["file", "size_bytes", "mtime_ns", "md5"])
        writer.writeheader()
