"""Local Nextflow runner for malaria-amplicon-nf."""
...
def nextflow_runtime_dirs() -> tuple[Path, Path]:
    """Return host-local Nextflow cache/temp paths that avoid mounted Windows paths."""
    configured = os.environ.get("SIMPLSEQ_RUNTIME_CACHE", "").strip()
    if configured:
        runtime_root = Path(configured).expanduser().resolve()
    elif os.name != "nt" and Path("/var/tmp").is_dir():
        uid = os.getuid() if hasattr(os, "getuid") else "user"
        runtime_root = Path("/var/tmp") / f"malaria-amplicon-nf-{uid}"
    else:
        runtime_root = Path(tempfile.gettempdir()) / "malaria-amplicon-nf"
    return runtime_root / "nextflow-home", runtime_root / "tmp"
...
def check_environment(
    ...
    *,
    ...
    if profile not in {"local", "reproducible"}:
        raise ValueError("malaria-amplicon-nf currently supports local Conda/Nextflow profiles only")
    ...
    probe_jobs = [
        ("Python packages", ["python", "-c", "import pandas; print('pandas ok')"], 120, None),
        ("Java works", ["java", "-version"], 30, None),
        ("Nextflow works", ["nextflow", "-version"], 30, None),
        (
            "DADA2 loads",
            ["Rscript", "-e", "suppressPackageStartupMessages(library(dada2)); cat(as.character(packageVersion('dada2')))"],
            60,
            None,
        ),
        ("MUSCLE works", ["muscle", "-version"], 30, {0, 1}),
        ("CD-HIT-EST works", ["cd-hit-est", "-h"], 30, {0, 1}),
    ]
    with ThreadPoolExecutor(max_workers=len(probe_jobs)) as executor:
        futures = [
            executor.submit(run_probe, name, command, env, timeout=timeout, ok_returncodes=ok_codes)
            for name, command, timeout, ok_codes in probe_jobs
        ]
    ...
    for rel in [
        ...
        "workflow/scripts/cdhit_clustering.py",
        "workflow/scripts/prepare_analysis_table.py",
...
def analysis_parameters(profile: str = "local") -> dict[str, object]:
    parameters: dict[str, object] = {
        ...
        "indel_filter": "per_locus_snv_filters",
        "cigar_homopolymer_mask_length": 5,
        "cigar_min_total_reads": 1,
        "cigar_min_samples": 1,
        "cigar_exclude_bimeras": True,
        "cdhit_identity": 0.989,
        "cdhit_word_size": 10,
...
def run_nextflow(
    ...
    *,
    ...
    pinned_parameters["inline_barcodes_enabled"] = bool(kelt_enabled)
    parameters = {
        "samples": str(samples),
        "outdir": str(outdir),
        ...
        "cigar_min_samples": 1,
        "cigar_exclude_bimeras": True,
        "kelt_enabled": bool(kelt_enabled),
        "kelt_barcode_map": str(kelt_barcode_map) if kelt_barcode_map else "",
    ...
    emit_event(progress_file, "run", "started", message="Starting malaria-amplicon-nf run")
    ...
    if kelt_enabled:
        command.extend(["--inline_barcodes_enabled", "true"])
    if cpus:
    ...
    env = local_runtime_env(root)
    nextflow_home, runtime_tmp = nextflow_runtime_dirs()
    nextflow_home.mkdir(parents=True, exist_ok=True)
    runtime_tmp.mkdir(parents=True, exist_ok=True)
    env["NXF_HOME"] = str(nextflow_home)
    env["NXF_TEMP"] = str(runtime_tmp)
    ...
    emit_event(progress_file, "run", status, message="malaria-amplicon-nf run finished")
...
def results_manifest(outdir: Path) -> dict[str, object]:
    ...
    files = [
        ...
        ("ASV filtering summary", outdir / "reports" / "asv_filtering_summary.txt"),
        ...
        ("CD-HIT cluster membership", outdir / "cdhit" / "cdhit_cluster_membership.tsv"),
        ("CD-HIT cluster count table", outdir / "cdhit" / "cdhit_cluster_counts.tsv"),
        ("CD-HIT representative FASTA", outdir / "cdhit" / "cdhit_representatives.fasta"),
        ("CD-HIT filtered ASV FASTA", outdir / "cdhit" / "filtered_ASVSeqs.fasta"),
        ("CD-HIT raw clusters", outdir / "cdhit" / "cdhit_representatives.fasta.clstr"),
        ("CD-HIT cluster summary", outdir / "cdhit" / "cdhit_summary.json"),
        ("KELT contamination report", outdir / "qc" / "kelt" / "kelt_contamination_report.html"),
        ("KELT contamination calls", outdir / "qc" / "kelt" / "kelt_contamination_calls.tsv"),
        ("KELT barcode counts", outdir / "qc" / "kelt" / "kelt_barcode_counts.tsv"),
        ("KELT contamination summary", outdir / "qc" / "kelt" / "kelt_contamination_summary.json"),
