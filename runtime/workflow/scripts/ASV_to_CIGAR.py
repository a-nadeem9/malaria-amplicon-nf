...
def _first_field(fieldnames, exact=(), prefix=()):
    lookup = {name.lower(): name for name in fieldnames}
    for name in exact:
        if name.lower() in lookup:
            return lookup[name.lower()]
    return next((name for name in fieldnames if any(name.lower().startswith(item.lower()) for item in prefix)), None)
...
def _integer(value, field, row_number):
    try:
        ...
        raise ValueError(f"Invalid {field} value on ASV table row {row_number}: {value!r}") from exc
...
def parse_asv_table(file, min_reads=0, min_samples=0, max_snv_dist=-1, max_indel_dist=-1, include_failed=False, exclude_bimeras=False):
    """Parse the named columns in a post-DADA2 ASV table."""
    bins = {}
    with open(file, newline="", encoding="utf-8-sig") as handle:
        reader = csv.DictReader(handle, delimiter="\t")
        fieldnames = reader.fieldnames or []
        asv_field = _first_field(fieldnames, exact=("hapid", "ASV"))
        reads_field = _first_field(fieldnames, exact=("total_reads",))
        samples_field = _first_field(fieldnames, exact=("total_samples",))
        amplicon_field = _first_field(fieldnames, exact=("amplicon",), prefix=("refid_",))
        snv_distance_field = _first_field(fieldnames, prefix=("snv_dist_from_",))
        indel_distance_field = _first_field(fieldnames, prefix=("indel_dist_from_",))
        snv_filter_field = _first_field(fieldnames, exact=("snv_filter",))
        indel_filter_field = _first_field(fieldnames, exact=("indel_filter",))
        bimera_field = _first_field(fieldnames, exact=("bimera",))
        required = {"ASV ID": asv_field, "total_reads": reads_field, "total_samples": samples_field, "amplicon/reference ID": amplicon_field}
        missing = [label for label, field in required.items() if not field]
        if missing:
            raise ValueError(f"ASV table is missing required column(s): {', '.join(missing)}")

        for row_number, row in enumerate(reader, start=2):
            if _integer(row[reads_field], reads_field, row_number) < min_reads:
                continue
            if _integer(row[samples_field], samples_field, row_number) < min_samples:
                continue
            if max_snv_dist >= 0 and snv_distance_field and _integer(row[snv_distance_field], snv_distance_field, row_number) > max_snv_dist:
                continue
            if max_indel_dist >= 0 and indel_distance_field and _integer(row[indel_distance_field], indel_distance_field, row_number) > max_indel_dist:
                continue
            filters = [row.get(field, "").strip().upper() for field in (snv_filter_field, indel_filter_field) if field]
            if not include_failed and "FAIL" in filters:
                continue
            if exclude_bimeras and bimera_field and row.get(bimera_field, "").strip().upper() == "TRUE":
                continue
            asv = row[asv_field].strip()
            amplicon = row[amplicon_field].strip()
            if asv and amplicon:
                bins.setdefault(amplicon, []).append(asv)
    return bins
...
def write_amplicon_fastas(asvs, bins, amplicons, outdir="ASVs"):
    ...
    for amplicon in bins:
        if amplicon not in amplicons:
            ...
            continue
        missing_asvs = [asv for asv in bins[amplicon] if asv not in asvs]
        if missing_asvs:
            raise ValueError(f"ASV table references ID(s) absent from FASTA: {', '.join(missing_asvs[:5])}")
...
def _get_homopolymer_runs(seq, min_length=5):
    ...
    # Runs followed by a different base are flushed in the loop. A run that
    # reaches the end of the reference needs the same treatment.
    if run >= min_length and start is not None and last_non_gap is not None:
        runs.update(range(start, last_non_gap + 1))
            ...
            if masked and (pos in masked or (pos+1 in masked and anchor[i] == '-')):
                ...
                if anchor[i] == "-":
                    if not indel:
                        ...
                        cigar += f"{pos}I="
                        if i:
                            for anchor_index in range(i - 1, -1, -1):
                                if anchor[anchor_index] != "-":
                                    cigar += anchor[anchor_index]
                                    break
                    cigar += seq[i]
                elif seq[i] == "-":
                    if not indel:
...
def parse_alignments(bins, mask={}, min_homopolymer_length=5, outdir="ASVs"):
    ...
    for amplicon in sorted(bins):
        ...
        parsed = parse_alignment(msa, mask=mask, min_homopolymer_length=min_homopolymer_length, amplicon=amplicon)
        if parsed is None:
            raise ValueError(f"Could not parse alignment for amplicon {amplicon}")
        cigars[amplicon] = parsed
...
def write_cigar_strings(cigars, out):
    ...
    with open(out, 'w') as w:
        ...
        for amplicon in sorted(cigars):
            # sort on ASV number, while still accepting non-standard IDs
            def asv_sort_key(value):
                match = number.search(value)
                return (int(match.group()) if match else sys.maxsize, value)
            for ASV in sorted(cigars[amplicon], key=asv_sort_key):
...
def convert_seqtab(file, cigars, out, asvs=None):
    ...
    with open(file) as f:
        ...
        for line in f:
            ...
            sample = line[0]
            if sample in seqtab:
                print(f"ERROR: Duplicate sample ID in seqtab: {sample}", file=sys.stderr)
                return
            seqtab[sample] = {}
            for asv, count in zip(resolved_asvs, line[1:]):
                ...
                try:
                    ...
                    print(f"ERROR: Non-integer ASV count for sample {sample}: {count!r}", file=sys.stderr)
                    return
                if count < 0:
                    print(f"ERROR: Negative ASV count for sample {sample}: {count}", file=sys.stderr)
                    return
...
def main():
    ...
    Path(outdir).mkdir(parents=True, exist_ok=True)
    write_amplicon_fastas(asvs, bins, amplicons, outdir=outdir)
