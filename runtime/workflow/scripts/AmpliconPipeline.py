...
def run_parallel_jobs(worker, jobs, max_workers=None):
	...
	if not jobs:
		return
	worker_count = max_workers or max_parallel_workers or min(len(jobs), os.cpu_count() or 1)
...
def sample_jobs(samples, *extra_args):
	...
	for row_number, sample in enumerate(samples, start=1):
		...
		if "\t" in line:
			...
			# Compatibility for historical three-column manifests without tabs.
			fields = line.split(maxsplit=2)
		fields = [field.strip() for field in fields]
		if len(fields) != 3 or not all(fields):
			sys.exit(f"Malformed metadata row {row_number}: expected sample ID, R1 path, and R2 path")
		jobs.append((fields[0], fields[1], fields[2], *extra_args))
	return jobs
...
def create_meta(path_to_fq,output_file,pattern_fw,pattern_rv):
	run_subprocess([sys.executable, os.path.join(path,'create_meta.py'),  '--path_to_fq', path_to_fq,
...
def main():
	...
	parser.add_argument('--workers', type=int, help="Maximum concurrent preprocessing jobs and DADA2 threads")

	args = parser.parse_args()
	env_workers = os.environ.get("SIMPLSEQ_THREADS", "").strip()
	requested_workers = args.workers if args.workers is not None else (int(env_workers) if env_workers.isdigit() else (os.cpu_count() or 1))
	if requested_workers < 1:
		parser.error("--workers must be at least 1")
	max_parallel_workers = requested_workers
	print(f"Using up to {max_parallel_workers} worker(s)")
	jsoninputs = {}
	if args.json is None:
		print('NOTE : JSON input file not provided. Checking for individual arguments')
	elif os.path.isfile(args.json):
		with open(args.json, 'r', encoding='utf-8') as jsonfile:
...
#### Adapter removal steps ####
	...
	if args.skip_preprocess:
		...
		with open(path_to_meta, 'r', encoding='utf-8') as meta:
	...
	if args.keep_primers:
		...
		if mark:
			...
			if args.iseq:
				...
				with open(os.path.join(run_dir,"iseq_temp_meta.txt"), 'r', encoding='utf-8') as temp_meta:
...
#### DADA2 Steps ####
	...
	if args.skip_dada2:
		...
		dada2_repro_args = []
		dada2_repro_args.extend(['--threads', str(max_parallel_workers)])
