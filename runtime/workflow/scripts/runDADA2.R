...
parser$add_argument("--threads", type="integer",
                    help="Maximum worker threads when multithreading is enabled")
...
if (is.null(work_dir) || !nzchar(work_dir)) {
  stop("Working directory (--dir) is required")
}
...
if (anyDuplicated(sample.names)) {
  stop("Duplicate sample IDs in metafile: ", paste(unique(sample.names[duplicated(sample.names)]), collapse = ", "))
}
missing_fastqs <- c(fnFs[!file.exists(fnFs)], fnRs[!file.exists(fnRs)])
if (length(missing_fastqs) > 0) {
  stop("FASTQ file(s) not found: ", paste(head(missing_fastqs, 5), collapse = ", "))
}
...
if (is.null(thread_count) || is.na(thread_count)) {
  env_threads <- suppressWarnings(as.integer(Sys.getenv("SIMPLSEQ_THREADS", unset = "1")))
  thread_count <- ifelse(is.na(env_threads), 1L, env_threads)
}
if (thread_count < 1) {
  stop("--threads must be at least 1")
}
...
quality_indices <- seq_len(min(2L, length(fnFs)))
png(paste0(work_dir,"/qualityF.png"), height = 800, width = 700)
try(print(plotQualityProfile(fnFs[quality_indices])), silent = TRUE)
dev.off()
png(paste0(work_dir,"/qualityR.png"),height = 800, width = 700)
try(print(plotQualityProfile(fnRs[quality_indices])), silent = TRUE)
dev.off()
...
if (length(sample.names) == 0) {
  stop("All samples had zero reads after DADA2 filtering. Review quality profiles and filter settings.")
}
...
if(args$bimera) {
  ...
  print(if (sum(seqtab) > 0) sum(seqtab.nochim)/sum(seqtab) else 0)
